...
public class EnglishStemmer implements Stemmer{
    ...
    private int maxWordLength = "antidisestablishmentarianism".length(); // or some other suitable value, e.g antidisestablishmentarianism
    ...
    private boolean setMaxWordLength( int length ) {
        maxWordLength = length;
        return true;
    }

    private boolean setMaxAcronymLength( int length ) {
    ...
    public String stem( String word ) {
        ...
        if( this.isProblemWord( word ) ) return word; // rule 90
        if( word.length() > maxWordLength ) return word; // rule 95
        if ( word.indexOf( "'" ) != -1 ) {                                     // word had apostrophe(s) - remove and continue 94
            ...
            return stemmed_word; // rule 94
        }


        //TODO maybe to conservative rule 2-6, and we are feeding lower case so who cares!

        if( word.matches( "^\\d+$" ) ) { return word; } // rule 90.3
        //else if( word.matches( "^\\w+-\\w+$" ) ) { return word; } // rule 90.2
       // else if( word.matches( "^.*-.*$" ) ) { return word; } // rule 90.1
       // else if( word.matches( "^.*_.*$" ) ) { return word; } // rule 90
       // else if( word.matches( "^\\p{Upper}+s$" ) ) { return this.remove( word, "s" ); } // rule 91.1
       // else if( word.matches( "^\\p{Upper}+$" ) ) { return word; } // rule 91
       // else if( word.matches( "^\\p{Upper}+$" ) ) { return word; } // rule 91
       // else if( word.matches( "^.*\\p{Upper}.*\\p{Upper}.*$" ) ) { return word; } // rule 92
       // else if( word.matches( "^\\p{Upper}{1}.*$" ) ) { return word; } // rule 92

        // should word be stemmed followed by call to private method with text to stem
        if (word.endsWith("aceous")) { stemmed_word = this.remove(word, "aceous"); } // 1
        else if (word.endsWith("ces")) { stemmed_word = this.remove(word, "s");  }  // 2
        ...
        return stemmed_word; // rule ruleno
    }
